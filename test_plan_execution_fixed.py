"""
Test Plan Execution Script for MLPY - Fixed Version
...
print("\n" + "="*80)
print("1. PRUEBAS DE FUNCIONALIDAD BASICA")
print("="*80)

# 1.1 Importación del Paquete
print("\n--- 1.1 Importacion del Paquete ---")
...
def test_validated_task():
    ...
    # Use validate_task_data function instead
    is_valid, issues = validate_task_data(data, target='y', task_type='classif')
    print(f"   Validation: valid={is_valid}, issues={len(issues)}")
...
test_wrapper("1.2.4 Validate task data", test_validated_task)
...
def test_generate_predictions():
    ...
    print(f"   Predictions: {len(predictions.response)} samples")
...
def test_measure_mse():
    ...
    task = TaskRegr(data=data, target='y')
    learner = LearnerDecisionTreeRegressor(random_state=42)
    learner.train(task)
    predictions = learner.predict(task)
...
# Export results for updating the markdown
import json
with open('test_results_basic.json', 'w') as f:
    json.dump(results, f, indent=2)
print("\nResultados guardados en test_results_basic.json")
